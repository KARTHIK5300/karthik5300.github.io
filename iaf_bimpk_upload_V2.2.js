...
const createBIMCollections = async (params, PlatformApi, ctx) => {
  ...
  let packagename = await PlatformApi.IafScriptEngine.getVar("package_name");
  let packagenameShort = await PlatformApi.IafScriptEngine.getVar("package_name_short");
  ...
  const model_els_coll = await PlatformApi.IafScriptEngine.createCollection(elementsCol, ctx);
  ...
  let elemIndex = await PlatformApi.IafScriptEngine.createOrRecreateIndex(elemCollIndex, ctx);
  ...
  const model_els_props_coll = await PlatformApi.IafScriptEngine.createCollection(modelElemPropsCol, ctx);
  ...
  const model_type_el_coll = await PlatformApi.IafScriptEngine.createCollection(typeElemsCol, ctx);
  ...
  let typeindex = await PlatformApi.IafScriptEngine.createOrRecreateIndex(typeElemCollIndex, ctx);
  ...
  const model_geom_file_coll = await PlatformApi.IafScriptEngine.createCollection(geometryFilesCol, ctx);
  ...
  const model_geom_views_coll = await PlatformApi.IafScriptEngine.createCollection(geometryViewsCol, ctx);
...
const data_cache_coll = await PlatformApi.IafScriptEngine.createCollection(dataCacheCol, ctx);
console.log(JSON.stringify({"message":"Create Model Data Cache"}));

let bimpkFileId = await PlatformApi.IafScriptEngine.getVar("bimpk_fileid");
let bimpkFileVersionId = await PlatformApi.IafScriptEngine.getVar("bimpk_fileVersionId");
...
console.log("fileid: " + bimpkFileId + ", bimpkFileVersionid: " + bimpkFileVersionId);
  ...
  let model = await PlatformApi.IafScriptEngine.createNamedCompositeItem(modelCompItem, ctx)
  await PlatformApi.IafScriptEngine.setVar("bim_model", model);
...
const createBIMCollectionVersion = async (params, PlatformApi, ctx) => {
  ...
  let bimModel = await PlatformApi.IafScriptEngine.getVar("bim_model")
  const modelRelatedCollection = await PlatformApi.IafScriptEngine.getCollectionsInComposite(bimModel._id,null, ctx);
  ...
  let version = await PlatformApi.IafScriptEngine.createNamedUserItemVersion(newModelVer,ctx);
  ...
  if (!data_cache_coll) {
    ...
    data_cache_coll = await PlatformApi.IafScriptEngine.createCollection(data_cache_coll_def, ctx);
    console.log(JSON.stringify({"message":"Create Model Data Cache"}));
  }

  // create the versions

  const model_els_coll_ver = await PlatformApi.IafScriptEngine.createNamedUserItemVersion({
  ...
  const model_els_props_coll_ver = await PlatformApi.IafScriptEngine.createNamedUserItemVersion({
  ...
  const model_type_el_coll_ver = await PlatformApi.IafScriptEngine.createNamedUserItemVersion({
  ...
  const data_cache_coll_ver = await PlatformApi.IafScriptEngine.createNamedUserItemVersion({
    "namedUserItemId": data_cache_coll._userItemId
  }, ctx);
  console.log(JSON.stringify({"message":"Create Data Cache Version data_cache"}));

  const model_geom_file_coll_ver = await PlatformApi.IafScriptEngine.createNamedUserItemVersion({
  ...
  const model_geom_views_coll_ver = await PlatformApi.IafScriptEngine.createNamedUserItemVersion({
  ...
  await PlatformApi.IafScriptEngine.createOrRecreateIndex(elemCollIndex, ctx);
  console.log(JSON.stringify({"message":"element index response"}));
  ...
  await PlatformApi.IafScriptEngine.createOrRecreateIndex(typeElemCollIndex, ctx);
  console.log(JSON.stringify({"message":"type index response"}));

  // set them in global variables
  PlatformApi.IafScriptEngine.setVar("model_els_coll", model_els_coll);
  PlatformApi.IafScriptEngine.setVar("model_els_props_coll", model_els_props_coll);
  PlatformApi.IafScriptEngine.setVar("model_type_el_coll", model_type_el_coll);
  PlatformApi.IafScriptEngine.setVar("data_cache_coll", data_cache_coll);
  PlatformApi.IafScriptEngine.setVar("model_geom_file_coll", model_geom_file_coll);
  PlatformApi.IafScriptEngine.setVar("model_geom_views_coll", model_geom_views_coll);
...
const createRelatedItemsAndRelationships = async (_colls, PlatformApi, ctx) => {
  ...
  await PlatformApi.IafScriptEngine.addRelatedCollections({
    "namedCompositeItemId": PlatformApi.IafScriptEngine.getVar("bim_model")._id,
    "relatedCollections": [
  ...
  const bim_els = await PlatformApi.IafScriptEngine.createItemsBulk({
    "_userItemId": _colls.model_els_coll._userItemId,
    "_namespaces": ctx._namespaces,
    "items": PlatformApi.IafScriptEngine.getVar("manage_els")
  }, ctx);
  console.log("Create Related Collection manage_els");

  const type_els = await PlatformApi.IafScriptEngine.createItemsBulk({
    "_userItemId": _colls.model_type_el_coll._userItemId,
    "_namespaces": ctx._namespaces,
    "items": PlatformApi.IafScriptEngine.getVar("manage_type_els")
  }, ctx);
  console.log("Create Related Collection manage_type_els");

  await PlatformApi.IafScriptEngine.createItemsAsRelatedBulk({
    "parentUserItemId": _colls.model_els_coll._userItemId,
    "_userItemId": _colls.model_els_props_coll._userItemId,
    "_namespaces": ctx._namespaces,
    "items": PlatformApi.IafScriptEngine.getVar("properties")
  },ctx);
  console.log("Create Related Collection properties");

  const el_to_type_relations = await PlatformApi.IafScriptEngine.createRelations({
    "parentUserItemId": _colls.model_els_coll._userItemId,
    "_userItemId": _colls.model_type_el_coll._userItemId,
    "_namespaces": ctx._namespaces,
    "relations": PlatformApi.IafScriptEngine.getVar("manage_el_to_type_relations")
  },ctx);
  console.log("Create Related Collection Relations");

  await PlatformApi.IafScriptEngine.setVar("outparams", {
    "filecolid": _colls.model_geom_file_coll._userItemId,
    "viewcolid": _colls.model_geom_views_coll._userItemId,
    "compositeitemid": PlatformApi.IafScriptEngine.getVar("bim_model")._id,
    "myCollections": _colls
  });
  return await PlatformApi.IafScriptEngine.getVar("outparams");
}
...
const extractBimpk = async (param, PlatformApi, ctx) => {
  ...
  try {
      ...
      try {
          assetTypeMap = await PlatformApi.IafScriptEngine.getItems({
                  "collectionDesc": {
                      "_userType": "iaf_dt_type_map_defs_coll",
                      "_namespaces": PlatformApi.IafScriptEngine.getVar("namespaces")
                  },
                  "options": {
                      "page": {
                          "getAllItems": true
      ...
      for (let type of _objectsArray.types) {
          ...
          type._id = await PlatformApi.IafScriptEngine.newID("mongo", {
                  "format": "hex"
              });
      ...
      for (let obj of _objectsArray.objects) {
          ...
          obj._id = await PlatformApi.IafScriptEngine.newID("mongo", {
                  "format": "hex"
              });
...
const cacheSourceFileGraphicsIds = async(params, PlatformApi, ctx) => {
  ...
  let sourcefiles = await PlatformApi.IafScriptEngine.getDistinct({
  ...
  for (let i = 0; i < sourcefileNames.length; i++) {
    ...
    let packageIds = await PlatformApi.IafScriptEngine.getDistinct({
      collectionDesc: { _userType: model_els_coll._userType, _userItemId: model_els_coll._userItemId },
...
export default {
  async uploadBimpk(params, PlatformApi, ctx) {
    ...
    await PlatformApi.IafScriptEngine.setVar("namespaces", ctx._namespaces);
    await PlatformApi.IafScriptEngine.setVar("package_name", param.filename);
    await PlatformApi.IafScriptEngine.setVar("package_name_short", param.filename.substring(0, 11));
    await PlatformApi.IafScriptEngine.setVar("bimpk_fileid", param._fileId);
    await PlatformApi.IafScriptEngine.setVar("bimpk_fileVersionId", param._fileVersionId);
    debugger;
    ...
    return PlatformApi.IafScriptEngine.getVar("outparams");

  },
  async createModelDataCache(params, PlatformApi, ctx) {

    await cacheSourceFileGraphicsIds(params, PlatformApi, ctx)
